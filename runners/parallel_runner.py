...
class ActThreaded(Thread):
    def __init__(self, should_stop, agent, env, parallel_buffer, index, config, num_steps, states, recurrent=False, internals=None,
                 v_internals=None, motivation=None, reward_model=None, random_actions=False):
    ...
    def run(self) -> None:
        ...
        for i in range(self.num_steps):
            ...
            if not self.recurrent:
                ...
                actions, _, epsilons, taus, logprobs, probs, dist = self.agent(torch.from_numpy(np.asarray([state])).to(self.agent.device).float())
            ...
            if self.random_actions:
                if self.agent.action_type == 'discrete':
                    actions = np.asarray(np.random.randint(self.agent.action_size))
                else:
                    actions = np.asarray(np.random.uniform(-1, 1, self.agent.action_size))
            entropies.append(self.env.entropy(dist))
            state_n, reward, done, info = self.env.step(actions)
            step += 1
            total_reward += reward
            ...
            state = state_n
            if done:
                self.parallel_buffer['episode_rewards'][self.index].append(self.env.ep_reward)
                self.parallel_buffer['episode_timesteps'][self.index].append(self.env.current_timestep)
...
class EpisodeThreaded(Thread):
    def __init__(self, should_stop, env, parallel_buffer, agent, index, config, num_episode=1, recurrent=False, motivation=False,
                 reward_model=False, motivation_model=None, random_actions=False):
        self.should_stop = should_stop
        self.env = env
        self.parallel_buffer = parallel_buffer
        self.agent = agent
    ...
    def run(self) -> None:
        ...
        for i in range(self.num_episode):
            ...
            while not done:
                ...
                actions, _, epsilons, taus, logprobs, probs, dist = self.agent(torch.from_numpy(np.asarray([state])).to(self.agent.device).float())
                actions = actions.detach().cpu().numpy()[0]
                logprobs = logprobs.detach().cpu().numpy() if logprobs is not None else None
                ...
                self.parallel_buffer['action'][self.index].append(actions)
                self.parallel_buffer['logprob'][self.index].append(logprobs)
                self.parallel_buffer['epsilons'][self.index].append(epsilons)
                self.parallel_buffer['taus'][self.index].append(taus)
...
class Runner:
    def __init__(self, should_stop, agent, frequency, envs, save_frequency=3000, logging=100, total_episode=1e10, curriculum=None,
                 frequency_mode='episodes', random_actions=None, curriculum_mode='steps', evaluation=False,
                 callback_function=None, motivation=None,
                 # IRL
                 reward_model=None, fixed_reward_model=False, dems_name='', reward_frequency=30, p_name_for_dems="", dem_loader=None,
        ...
        # If we want to use SMP - diffusion prior
        self.diffusion_prior = diffusion_prior
        self.timesteps_set = timesteps_set
        ...
        if self.adversarial_play:
            self.agent.save_model(name=self.agent.model_name + '_0', folder='saved/adversarial', with_onnx=False)
            self.double_agent.load_model(name=self.agent.model_name + '_0', folder='saved/adversarial', with_onnx=False)
        ...
        if self.reward_model is not None:
            if self.dem_loader is not None:
                dems = dem_loader.load_demonstrations_txt(self.dems_name)
                self.reward_model.set_demonstrations(dems, None)
            elif not self.fixed_reward_model:
                # Ask for demonstrations
                answer = None
                while answer != 'y' and answer != 'n' and answer != 'p':
                ...
                if answer == 'y':
                    dems, vals = self.reward_model.create_demonstrations(env=self.envs[0], dems_name=self.dems_name)
                elif answer == 'p':
                    agent.load_model(name=p_name_for_dems, folder='saved')
                    dems, vals = self.reward_model.create_demonstrations(env=self.envs[0], with_policy=True,
                                                                         dems_name=self.dems_name, policy=agent)
                    print("The demonstrations are saved. Now I will kill this process, restart it without creating"
                          "new examples.")
                    raise Exception("Restart the process!")
                else:
                    print('Loading demonstrations...')
                    dems, vals = self.reward_model.load_demonstrations(self.dems_name)

                print('Demonstrations loaded! We have ' + str(
                    len(dems['obs'])) + " timesteps in these demonstrations")
        ...
        # self.agent.save_model(self.agent.model_name)

    # Return a list of thread, that will save the experience in the shared buffer
    # The thread will run for 1 episode
    def create_episode_threads(self, should_stop, parallel_buffer, agent, config, random_actions=False):
        # The number of thread will be equal to the number of environments
        threads = []
        for i, e in enumerate(self.envs):
            # Create a thread
            threads.append(EpisodeThreaded(should_stop, env=e, index=i, agent=agent, parallel_buffer=parallel_buffer, config=config,
                                           recurrent=self.recurrent, motivation=(self.motivation is not None),
                                           reward_model=(self.reward_model is not None),
                                           motivation_model=self.motivation, random_actions=random_actions))
    ...
    def create_act_threds(self, should_stop, agent, parallel_buffer, config, states, num_steps, internals=None, v_internals=None, random_actions=False):
        # The number of thread will be equal to the number of environments
        threads = []
        for i, e in enumerate(self.envs):
            # Create a thread
            threads.append(ActThreaded(should_stop, agent=agent, env=e, index=i, parallel_buffer=parallel_buffer, config=config,
                                       ...
                                       reward_model=(self.reward_model is not None), random_actions=random_actions))
    ...
    def clear_parallel_buffer(self):
        ...
        for i in range(len(self.envs)):
            ...
            parallel_buffer['epsilons'].append([])
            parallel_buffer['taus'].append([])
    ...
    def run(self):
        ...
        while self.ep <= self.total_episode and not self.should_stop.is_set():
            ...
            if self.frequency_mode == 'episodes':
                ...
                threads = self.create_episode_threads(self.should_stop, agent=self.agent, parallel_buffer=self.parallel_buffer,
                                                      config=config, random_actions=(self.random_actions is not None and self.total_step < self.random_actions))
                ...
                # Create threads
                if self.recurrent:
                    threads = self.create_act_threds(self.should_stop, agent=self.agent, parallel_buffer=self.parallel_buffer,
                                                     config=config,
                                                     states=states, num_steps=self.frequency//len(self.envs), internals=internals,
                                                     v_internals=v_internals, random_actions=(self.random_actions is not None and self.total_step < self.random_actions))
                else:
                    threads = self.create_act_threds(self.should_stop, agent=self.agent, parallel_buffer=self.parallel_buffer,
                                                     config=config,
                                                     states=states, num_steps=self.frequency//len(self.envs), random_actions=(self.random_actions is not None and self.total_step < self.random_actions))
                ...
                self.total_step += self.frequency
            ...
            for i in range(len(self.envs)):
                ...
                if not self.recurrent:
                    ...
                    for state, state_n, action, reward, logprob, done, epsilon, tau in zip(
                            ...
                            self.parallel_buffer["epsilons"][i],
                            self.parallel_buffer["taus"][i]
                    ):
                        self.agent.add_to_buffer(state, state_n, action, reward, logprob, done, epsilons=epsilon, taus=tau)
                else:
                    # Add to the agent experience in order of execution
            ...
            if not self.evaluation and self.frequency_mode == 'timesteps' and \
                ...
                self.compute_intrinsic_rewards(self.agent, False)
            ...
            # Logging information
            if self.frequency_mode == 'episodes' and self.ep > 0 and self.ep % self.logging == 0 or \
            self.frequency_mode == 'timesteps' and self.total_step > 0 and self.total_step % self.logging == 0:
            # if True:
                if self.frequency_mode == 'episodes':
                    print('Mean of {} episode reward after {} episodes: {}'.
                          format(self.logging, self.ep, np.mean(self.history['episode_rewards'][-self.logging:])))
                else:
                    print('Mean reward of last 100 episodes after {} timesteps: {}'.
                          format(self.logging, np.mean(self.history['episode_rewards'][-100:])))
                    print('Reward of last episode: {}'.
                          format(np.mean(self.history['episode_rewards'][-1])))
                    print('Episodes completed: {}'.
                          format(len(self.history['episode_rewards'])))
            ...
            if self.frequency_mode == 'episodes' and self.ep > 0 and self.ep % self.save_frequency == 0 or \
                    self.frequency_mode == 'timesteps' and self.total_step > 0 and self.total_step % self.save_frequency == 0:
                self.save_model(self.history, self.agent.model_name, self.curriculum, self.agent)

            # if self.should_stop.is_set():
            #     break

    def save_model(self, history, model_name, curriculum, agent, with_timestep=False):

        # Save statistics as json
        json_str = json.dumps(history, cls=NumpyEncoder)
        if with_timestep:
            f = open("arrays/{}_{}.json".format(model_name, len(history['episode_timesteps'])), "w")
        else:
        ...
        # Save the tf model
        if with_timestep:
            agent.save_model(name='{}_{}'.format(model_name, len(history['episode_timesteps'])), folder='saved',
                             with_onnx=False)
        else:
    ...
    def get_experience(self, env, num_discriminator_exp=None, verbose=False, random=False):
        ...
        for ep in range(num_discriminator_exp):
            ...
            while True:
                ...
                state_n, step_reward, terminal, info = env.step(actions=action)
                if self.reward_model is not None:
    ...
    def compute_intrinsic_rewards(self, agent, with_extrinsic=False):
        if self.motivation is not None:
            ...
            # Compute intrinsic rewards
            num_batches = 10
            batch_size = int(np.ceil(len(agent.buffer['states_n']) / num_batches))
            intrinsic_rews = []
            for i in range(num_batches):
                c_intrinsic_rews = self.motivation.eval(deepcopy(
                    agent.buffer['states_n'][batch_size * i:batch_size * i + batch_size]))
            ...
            if with_extrinsic:
                agent.buffer['rewards'] = list(intrinsic_rews + np.asarray(agent.buffer['rewards']))
            else:
                agent.buffer['rewards'] = list(intrinsic_rews)
        ...
        if self.reward_model is not None:
            ...
            if with_extrinsic:
                agent.buffer['rewards'] = list(intrinsic_rews + np.asarray(self.agent.buffer['rewards']))
            else:
                agent.buffer['rewards'] = list(intrinsic_rews)
