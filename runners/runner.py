...
class Runner:
    ...
    def run(self):
        ...
        while self.ep <= self.total_episode and not self.should_stop.is_set():
            ...
            self.env.set_config(config)
            traj = dict(states=[], actions=[], rewards=[], dones=[], next_states=[])
            ...
            while True:
                ...
                if not self.recurrent:
                    ...
                    action, _, epsilons, taus, logprob, probs, dist = self.agent(torch.from_numpy(np.asarray([state])).to(self.device).float(), train=True)
                else:
                ...
                if self.random_actions is not None and self.total_step < self.random_actions:
                    if self.agent.action_type == 'discrete':
                        action = np.asarray(np.random.randint(self.agent.action_size))
                    else:
            ...
            if not self.evaluate and self.ep > 0 and self.ep % self.save_frequency == 0:
