import pdb
import pickle
import argparse
import matplotlib.pyplot as plt
import glob
import pandas as pd
import re
import json

import numpy as np

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # Experiments, we will plot more experiments in the same plot (for each value)
    parser.add_argument("-rs", "--runs", default="td3bc")  # Name of the experiments
    parser.add_argument("-f", "--folder", default="arrays")  # Name of the Folder
    parser.add_argument("-ma", "--moving-average", default=1000, type=int)  # COM value for rewards moving average
    parser.add_argument("-e", "--episodes", default=None, type=int) # Number of episodes to consider 

    parser.add_argument('-t', '--with-timesteps', action='store_true')
    parser.set_defaults(with_timesteps=False)

    args = parser.parse_args()

    # Get the real name of the files
    runs = args.runs
    runs = runs.replace(' ', '')
    runs = runs.replace('.json', '')
    runs = runs.split(",")

    filenames = []
    for run_name in runs:
        path = glob.glob(f"{args.folder}/{run_name}.json")
        for filename in path:
            if 'curriculum.json' in filename:
                continue
            with open(filename, 'r') as f:
                filenames.append(filename)
    print(filenames)

    if len(filenames) == 0:
        raise Exception(f"There are no files in the folder *{args.folder}* with names: {run_name}")

    print(f"Here are the file names to plot: {filenames}")

    # Create a dict for numeric table
    numeric_values = dict()

    # For each file name, load the stats
    stats = []
    for run in filenames:
        # Load stats file
        with open(run, 'rb') as handle:
            try:
                stat = json.load(handle)
                stat['name_run'] = run
                # If there is info, change from dict to list 
                if "info" in stat.keys() and stat["info"] is not None and "success_rate" in stat["info"][0]:
                    stat["success_rate"] = np.asarray([info["success_rate"] for info in  stat["info"]]) == 1

                    if "time" in stat["info"][0]:
                        stat["time"] = np.asarray([info["time"] for info in  stat["info"]])
                    
                    if "real_outcome" in stat["info"][0]:
                         stat["real_outcome"] = np.asarray([info["real_outcome"] for info in  stat["info"]])
                    
                    if "line_reward_component" in stat["info"][0]:
                         stat["line_reward_component"] = np.asarray([info["line_reward_component"] for info in  stat["info"]])
                    if "terminal_reward_component" in stat["info"][0]:
                         stat["terminal_reward_component"] = np.asarray([info["terminal_reward_component"] for info in  stat["info"]])
                    if "negative_reward_component" in stat["info"][0]:
                         stat["negative_reward_component"] = np.asarray([info["negative_reward_component"] for info in  stat["info"]])
                    if "movement_reward_component" in stat["info"][0]:
                         stat["movement_reward_component"] = np.asarray([info["movement_reward_component"] for info in  stat["info"]])

                # stat["success_rate"] = np.asarray(stat["episode_rewards"]) > 9
                stats.append(stat)
            except Exception as e:
                print(e)
                continue
    # Group stats per name
    stats_dict = dict()
    filenames = []
    for s in stats:
        name_run = s['name_run']
        name_run = re.sub("x-\d", "x", name_run)
        name_run = re.sub("-\d-", "-", name_run)
        name_run = re.sub("-\d$", "", name_run)
        if name_run in stats_dict:
            stats_dict[name_run].append(s)
        else:
            stats_dict[name_run] = [s]
            name_run = name_run.replace("-no-expert", "")
            name_run = re.sub(r"-exp+$", "", name_run)
            filenames.append(name_run)
    

    # Names of the data to plot and plot titels
    data_names = ['episode_rewards', 'mean_entropies', "success_rate", "episode_timesteps"]
    title_names = ['Environment Rewards', 'Entropy', "Success Rate", "Episode Timesteps"]

    # Plot all data in different figures
    for data_name, title_name in zip(data_names, title_names):
        plt.figure()

        for run_name, stats in stats_dict.items():
            all_datas = []
            all_raw_datas = []

            for s in stats:
                try:
                    data = s[data_name]
                    data = np.asarray(data).reshape(-1)
                    data = pd.DataFrame({'data': data})
                    all_raw_datas.append(data)
                    data = data.ewm(com=args.moving_average).mean()
                    all_datas.append(data)
                except Exception as e:
                    pass
            
            all_datas = np.asarray(all_datas)
            if len(all_datas.shape) == 1:
                continue
            
            episodes = all_datas.shape[1]
            if args.episodes is not None:
                episodes = args.episodes
            
            # Get the timesteps
            timesteps = stats[0]["episode_timesteps"][:episodes]
            timesteps = np.cumsum(timesteps)

            all_raw_datas[0] = all_raw_datas[0][:episodes]
            all_datas = all_datas[:, :episodes, :]

            # Compute means and stds of runs
            means = np.mean(all_datas, axis=0)
            stds = np.std(all_datas, axis=0)

            plt.title(title_name)
            if title_name == 'D4RL Rewards':
                plt.ylim(0, 120)

            # Decide whether to print based on episodes or timesteps
            x_axis = np.arange(len(means))
            if args.with_timesteps:
                x_axis = timesteps

            plt.plot(x_axis, means)
            means = np.reshape(means, (-1,))
            stds = np.reshape(stds, (-1,))

            plt.fill_between(x_axis, means - stds, means + stds, alpha=0.5, linewidth=0, label='_nolegend_')
            idx_of_max_mean = np.argmax(means)

            if title_name == "Episode Timesteps":
                pass
            else:
                print(f"Mean and STD of the latest {args.moving_average} episodes of {data_name} for {run_name}: {np.mean(all_raw_datas[0].values[-args.moving_average:])} +- {np.std(all_raw_datas[0].values[-args.moving_average:])}")

        plt.legend(filenames)

    data_names = ["episode_rewards", "line_reward_component", "terminal_reward_component", "negative_reward_component", "movement_reward_component"]
    title_names = ["Episode Rewards", "Line Reward Component", "Terminal Reward Component", "Negative Reward Component", "Movement Reward Component"]

    # Plot the reward component
    for run_name, stats in stats_dict.items():
        if data_names[1] not in stats[0]:
            continue
        plt.figure()
        all_datas = []

        for st in stats:
            for data_name in data_names:
                if data_name not in st:
                    continue
                data = st[data_name]
                data = pd.DataFrame({'data': data})
                all_raw_datas.append(data)
                data = data.ewm(com=args.moving_average).mean()
                all_datas.append(data)
        
        all_datas = np.asarray(all_datas)
        episodes = all_datas.shape[1]

        if args.episodes is not None:
            episodes = args.episodes

        # Get the timesteps
        timesteps = stats[0]["episode_timesteps"][:episodes]
        timesteps = np.cumsum(timesteps)

        plt.title(run_name)

        all_datas = all_datas[:, :episodes, :]
        for compo in all_datas:
            
            # Decide whether to print based on episodes or timesteps
            x_axis = np.arange(len(compo))
            if args.with_timesteps:
                x_axis = timesteps

            plt.plot(x_axis, compo.reshape(-1))
            plt.fill_between(x_axis, 0, compo.reshape(-1), alpha=0.5, linewidth=0, label='_nolegend_')
        plt.legend(title_names)

    # This scripts aggregate multiple runs with the same name. We don't have them here in FC, so we take only the 0th index
    for stat in stats_dict.values():
        print(f"Time and Success Rate for run with name: {stat[0]['name_run']}")
        try:
            # Print time passed
            hours, rem = divmod(stat[0]['time'][args.episodes - 1 if args.episodes is not None else -1], 3600)
            minutes, seconds = divmod(rem, 60)
            print("Time passed: {:0>2}:{:0>2}:{:05.2f}".format(int(hours), int(minutes), seconds))
        except Exception:
            print("No time stored for this experiment")
        
        try:
            # Print more detailed outcome
            # -1 goal, 0 max step, 1 block, 2 deflect, 3 team, 4 oop
            outcome_categories = ["goal", "max step", "block", "deflect", "team", "oop"]
            for index, category in zip(range(-1, 5), outcome_categories):
                percentage = np.count_nonzero(stat[0]["real_outcome"][-args.moving_average:] == index)/len(stat[0]["real_outcome"][-args.moving_average:])
                print(f"Percentage of {outcome_categories[index + 1]}: {percentage}")
        except Exception:
            print("No real outcome stored for this experiment")    
            

    # Show the images
    plt.show()